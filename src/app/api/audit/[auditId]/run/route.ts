...
// Use NEXT_PUBLIC_APP_URL (set in Render env) for internal server-to-server calls.
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const runDimension = async (dim: string): Promise<{ success: boolean; score?: number | null }> => {
        ...
        for (const dim of ['d1', 'd2', 'd3', 'd4', 'd5', 'd6', 'd7', 'd8', 'd9', 'd10']) {
          ...
          try {
            ...
            // Retry once after 5 s if the first attempt failed
          ...
          // Small pause between dimensions to avoid hitting Groq rate limits
