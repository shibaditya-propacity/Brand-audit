...
export function SentimentWidget({ findings }: SentimentWidgetProps) {
  ...
  return (
    <Card className="bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 p-4 space-y-4">
      <h3 className="font-semibold text-sm text-slate-900 dark:text-slate-100">Review Sentiment</h3>
      ...
      {(f.positiveThemes?.length > 0 || f.negativeThemes?.length > 0) && (
        <div className="grid grid-cols-2 gap-3">
          {f.positiveThemes?.length > 0 && (
            <div>
              <p className="text-xs font-medium text-emerald-600 dark:text-emerald-400 mb-1">Positive Themes</p>
              <ul className="space-y-0.5">
                {f.positiveThemes.slice(0, 4).map((t, i) => <li key={i} className="text-xs text-slate-600 dark:text-slate-300">• {t}</li>)}
              </ul>
            </div>
          )}
          {f.negativeThemes?.length > 0 && (
            <div>
              <p className="text-xs font-medium text-rose-600 dark:text-rose-400 mb-1">Negative Themes</p>
              <ul className="space-y-0.5">
                {f.negativeThemes.slice(0, 4).map((t, i) => <li key={i} className="text-xs text-slate-600 dark:text-slate-300">• {t}</li>)}
              </ul>
            </div>
          )}
        </div>
