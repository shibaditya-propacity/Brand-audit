...
export function LogoAuditWidget({ logoUrl, logoAnalysis }: LogoAuditWidgetProps) {
  ...
  return (
    <div className="rounded-lg border bg-white p-4 space-y-4">
      <h3 className="font-semibold text-sm">Logo Audit</h3>
      <div className="flex items-center gap-4">
        {logoUrl ? (
          ...
          <div className="h-16 w-16 rounded-lg border bg-gray-100 flex items-center justify-center text-xs text-muted-foreground">No logo</div>
        )}
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={item.label} className="flex items-center gap-2">
              ...
              <span className="text-xs font-medium w-6 text-right">{item.score}/10</span>
            </div>
          ))}
        </div>
      )}
