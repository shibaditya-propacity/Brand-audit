...
export default function D6Page({ params }: { params: { auditId: string } }) {
  ...
  const leftContent = (
    <>
      ...
      <ManualInputForm
        ...
        initialData={audit?.manualOverrides?.['D6'] as Record<string, unknown> | undefined}
        onSaved={refetch}
      />
    </>
  );
  ...
  return (
    <AppShell auditId={params.auditId} dimensionScores={dimensionScores}>
      <DimensionPageShell dimensionCode="D6" dimension={dimension} leftContent={leftContent} rightContent={rightContent} auditId={params.auditId} onRerunComplete={refetch} />
    </AppShell>
  );
}
